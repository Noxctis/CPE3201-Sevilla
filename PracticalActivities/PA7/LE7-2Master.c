 ...
 * @file I2C_Master.c
 * @author cssevilla
 * @brief I2C Master Implementation - Transmits PORTD and Receives to PORTB
 */
...
/**
 * @brief Waits for the I2C bus and MSSP hardware to become idle
 */
void I2C_Wait(void) 
{ 
...
/**
 * @brief Initializes the PIC as an I2C Master at 100kHz
 */
 ...
 * @param ack Set to 1 to send ACK (more data requested), 0 for NACK (last byte)
...
{ 
    // --- I/O Setup ---
    TRISB = 0x00;   // PORTB as Output (Displays data received from Slave)
    PORTB = 0x00;   
    TRISD = 0xFF;   // PORTD as Input (Reads data to transmit to Slave)
    ...
    {  
        // --------------------------------------------------------
        // SEQUENCE 1: Master Write Operation (Send PORTD to Slave)
        // --------------------------------------------------------
        I2C_Start();     
        // 0x10 = Slave Address (0x08 shifted left by 1) + Write bit (0)
        I2C_Send(0x10);  
        I2C_Send(PORTD); // Transmit the state of PORTD buttons/switches
        I2C_Stop();      
        
        __delay_ms(200); // Polling delay

        // --------------------------------------------------------
        // SEQUENCE 2: Master Read Operation (Receive to PORTB)
        // --------------------------------------------------------
        I2C_Start();     
        // 0x11 = Slave Address (0x08 shifted left by 1) + Read bit (1)
        I2C_Send(0x11);  
        // Receive 1 byte from Slave, send NACK (0) to end transmission
        PORTB = I2C_Receive(0); 
        I2C_Stop();      
